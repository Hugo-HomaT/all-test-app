...
public class ArrowDirection : MonoBehaviour
{
	...
	private void Update()
	{
		if (arrowPivot == null || _activeArrowModel == null)
		{
			return;
		}
		if (!_isEnabled)
		...
		if (_currentTarget != null && _currentTarget.gameObject.activeInHierarchy)
		{
			...
			float horizontalDistance = new Vector3(targetPos.x - myPos.x, 0f, targetPos.z - myPos.z).magnitude;
	...
	private SupportActivator FindClosestTopItem()
	{
		...
		SupportActivator[] array = allActivators;
		foreach (SupportActivator activator in array)
		{
			if (!(activator == null) && activator.gameObject.activeInHierarchy && activator.isTopItem)
			{
				Vector3 targetPos = activator.transform.position;
				float horizontalDistance = new Vector3(targetPos.x - myPosition.x, 0f, targetPos.z - myPosition.z).magnitude;
				if (!(horizontalDistance < minDistance) && horizontalDistance < closestDistance)
				{
	...
	private void RotateTowardTarget()
	{
		...
		{
			Vector3 directionToTarget = _currentTarget.transform.position - base.transform.position;
			directionToTarget.y = 0f;
			if (!(directionToTarget.sqrMagnitude < 0.001f))
			{
				Quaternion targetRotation = Quaternion.LookRotation(directionToTarget.normalized, Vector3.up);
				arrowPivot.rotation = Quaternion.RotateTowards(arrowPivot.rotation, targetRotation, rotationSpeed * Time.deltaTime);
			}
		}
	}
